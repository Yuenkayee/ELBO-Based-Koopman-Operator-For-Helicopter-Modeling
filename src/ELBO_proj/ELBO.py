...
class ELBO(nn.Module):
    ...
    def forward(self, X_seq, U_seq):
        ...
        for t in range(self.T):
            ...
            x_roll = self.nn_C(z_roll)
            X_rollout_list.append(x_roll)

        X_rollout = torch.stack(X_rollout_list, dim=1).reshape(
            X_seq.shape[0], self.T * self.x_dim
        )
        rollout_loss = F.mse_loss(X_rollout, X_seq)
